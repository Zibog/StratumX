...
`Model Adapters` is a canonical element of `engine_inference` inside `L3.0. Model Systems`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Adapter layer that binds engine inference contracts to concrete model backends.
