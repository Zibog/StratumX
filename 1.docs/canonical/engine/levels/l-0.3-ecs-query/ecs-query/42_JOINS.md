...
Multi-set traversal composition.

## Canonical Definition

`Joins` is a canonical element of `engine_ecs_query` inside `L-0.3. ECS Query`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Join rules combining multiple ECS subsets into one legal query surface.
