...
`Lighting` is a canonical element of `engine_imaging` inside `L3.1. Synthesis Systems`. It exists as one explicit part of the engine stack and does not transfer its ownership class into another crate or level.

## Data Model

Lighting inputs and lighting-side synthesis structures for final image formation.
