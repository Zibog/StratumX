# Dependencies

## Legal dependencies
- tooling workspace_runtime
- viewport system
- workspace layout system
- assistant/diagnostics/build surfaces

## Forbidden dependencies
- undeclared lower-stack private internals
- runtime or tooling truth ownership that belongs elsewhere
- hidden side channels bypassing commands, requests, or declared refs

## Dependency law
The dependency contour above is specific to `editor_shell` and may not be widened by analogy to neighboring levels.
