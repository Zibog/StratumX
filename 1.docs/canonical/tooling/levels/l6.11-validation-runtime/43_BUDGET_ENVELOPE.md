...
## Budget posture for `validation_runtime`
- correctness of `validation_run_id`, `validation_scope` is non-degradable
- lower-priority outputs such as validation issue sets; validation status streams degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.11-validation-runtime` / `43_BUDGET_ENVELOPE.md`.
