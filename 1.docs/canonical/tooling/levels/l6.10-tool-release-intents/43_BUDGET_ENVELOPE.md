...
## Budget posture for `tool_release_intents`
- correctness of `release_intent_id`, `intent_kind` is non-degradable
- lower-priority outputs such as published `tool_release_intents` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.10-tool-release-intents` / `43_BUDGET_ENVELOPE.md`.
