...
## Budget posture for `tool_activation_state`
- correctness of `activation_state_id`, `tool_kind` is non-degradable
- lower-priority outputs such as published `tool_activation_state` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.13-tool-activation-state` / `43_BUDGET_ENVELOPE.md`.
