...
## Budget posture for `stream_plane`
- correctness of `stream_event_id`, `stream_kind` is non-degradable
- lower-priority outputs such as diagnostic streams; preview status streams degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.7-stream-plane` / `43_BUDGET_ENVELOPE.md`.
