...
## Required verification for `tool_task_requests`
- invariant tests for `task_request_id`, `task_kind`, `target_scope`
- dependency legality tests proving `{key}` resolves only `l6.0-authority-core`, `l6.0-tool-session`
- communication tests covering published `tool_task_requests` rows for downstream services; bounded status or routing updates where applicable
- concurrency tests for the writer/reader posture of `{key}`

## Operational note
This file remains active and package-specific for `l6.14-tool-task-requests` / `42_TEST_SURFACES.md`.
