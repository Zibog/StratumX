...
## Budget posture for `build_runtime`
- correctness of `build_job_id`, `build_target_set` is non-degradable
- lower-priority outputs such as built artifacts; build progress streams degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.13-build-runtime` / `43_BUDGET_ENVELOPE.md`.
