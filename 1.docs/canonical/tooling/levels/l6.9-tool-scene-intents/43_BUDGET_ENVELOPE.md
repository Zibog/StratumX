...
## Budget posture for `tool_scene_intents`
- correctness of `scene_intent_id`, `intent_kind` is non-degradable
- lower-priority outputs such as published `tool_scene_intents` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.9-tool-scene-intents` / `43_BUDGET_ENVELOPE.md`.
