...
## Budget posture for `transaction_ledger`
- correctness of `transaction_id`, `command_envelope_id` is non-degradable
- lower-priority outputs such as transaction records; rollback/revert inputs degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.2-transaction-ledger` / `43_BUDGET_ENVELOPE.md`.
