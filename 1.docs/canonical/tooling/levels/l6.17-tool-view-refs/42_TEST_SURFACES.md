...
## Required verification for `tool_view_refs`
- invariant tests for `view_ref_id`, `view_kind`, `target_scope`
- dependency legality tests proving `{key}` resolves only `l6.0-authority-core`, `l6.0-tool-session`
- communication tests covering published `tool_view_refs` rows for downstream services; bounded status or routing updates where applicable
- concurrency tests for the writer/reader posture of `{key}`

## Operational note
This file remains active and package-specific for `l6.17-tool-view-refs` / `42_TEST_SURFACES.md`.
