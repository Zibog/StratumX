...
## Budget posture for `tool_view_refs`
- correctness of `view_ref_id`, `view_kind` is non-degradable
- lower-priority outputs such as published `tool_view_refs` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.17-tool-view-refs` / `43_BUDGET_ENVELOPE.md`.
