...
## Budget posture for `tool_focus_refs`
- correctness of `focus_ref_id`, `focused_target_ref` is non-degradable
- lower-priority outputs such as published `tool_focus_refs` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.2-tool-focus-refs` / `43_BUDGET_ENVELOPE.md`.
