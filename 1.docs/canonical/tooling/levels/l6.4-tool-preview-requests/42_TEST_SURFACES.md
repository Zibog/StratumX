...
## Required verification for `tool_preview_requests`
- invariant tests for `preview_request_id`, `request_kind`, `target_ref_set`
- dependency legality tests proving `{key}` resolves only `l6.0-authority-core`, `l6.0-tool-session`
- communication tests covering published `tool_preview_requests` rows for downstream services; bounded status or routing updates where applicable
- concurrency tests for the writer/reader posture of `{key}`

## Operational note
This file remains active and package-specific for `l6.4-tool-preview-requests` / `42_TEST_SURFACES.md`.
