...
## Budget posture for `tool_preview_requests`
- correctness of `preview_request_id`, `request_kind` is non-degradable
- lower-priority outputs such as published `tool_preview_requests` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.4-tool-preview-requests` / `43_BUDGET_ENVELOPE.md`.
