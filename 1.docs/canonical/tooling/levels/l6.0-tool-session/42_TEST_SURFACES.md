...
## Required verification for `tool_session`
- invariant tests for `tool_session_id`, `session_origin`, `attachment_scope`
- dependency legality tests proving `{key}` resolves only `l6.0-authority-core`, `l6.0-tool-session`
- communication tests covering published `tool_session` rows for downstream services; bounded status or routing updates where applicable
- concurrency tests for the writer/reader posture of `{key}`

## Operational note
This file remains active and package-specific for `l6.0-tool-session` / `42_TEST_SURFACES.md`.
