...
## Required verification for `tool_inspection_views`
- invariant tests for `inspection_view_id`, `target_ref`, `field_set_ref`
- dependency legality tests proving `{key}` resolves only `l6.0-authority-core`, `l6.0-tool-session`
- communication tests covering published `tool_inspection_views` rows for downstream services; bounded status or routing updates where applicable
- concurrency tests for the writer/reader posture of `{key}`

## Operational note
This file remains active and package-specific for `l6.3-tool-inspection-views` / `42_TEST_SURFACES.md`.
