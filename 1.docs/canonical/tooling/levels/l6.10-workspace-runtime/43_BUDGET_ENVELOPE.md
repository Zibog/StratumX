...
## Budget posture for `workspace_runtime`
- correctness of `workspace_session_id`, `published_selection_ref_set` is non-degradable
- lower-priority outputs such as public coordination refs; runtime-attach publications degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.10-workspace-runtime` / `43_BUDGET_ENVELOPE.md`.
