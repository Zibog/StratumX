...
## Budget posture for `tool_diagnostics_events`
- correctness of `diagnostic_event_id`, `diagnostic_kind` is non-degradable
- lower-priority outputs such as published `tool_diagnostics_events` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.6-tool-diagnostics-events` / `43_BUDGET_ENVELOPE.md`.
