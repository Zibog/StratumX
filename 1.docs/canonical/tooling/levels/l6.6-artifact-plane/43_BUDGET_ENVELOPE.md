...
## Budget posture for `artifact_plane`
- correctness of `artifact_id`, `artifact_kind` is non-degradable
- lower-priority outputs such as artifact manifests; reverse references to generated outputs degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.6-artifact-plane` / `43_BUDGET_ENVELOPE.md`.
