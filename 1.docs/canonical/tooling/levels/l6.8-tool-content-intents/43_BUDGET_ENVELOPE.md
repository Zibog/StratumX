...
## Budget posture for `tool_content_intents`
- correctness of `content_intent_id`, `intent_kind` is non-degradable
- lower-priority outputs such as published `tool_content_intents` rows for downstream services; bounded status or routing updates where applicable degrade before correctness
- any drop or defer decision must be visible through `{key}` publications, never hidden in an unnamed cache

## Operational note
This file remains active and package-specific for `l6.8-tool-content-intents` / `43_BUDGET_ENVELOPE.md`.
