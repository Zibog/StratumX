...
const TOOL_CONFIG_DIRS: &[&str] = &[".qwen", ".cursor", ".windsurf"];
...
pub fn format_violations(violations: &[CleanlinessViolation]) -> String {
    if violations.is_empty() {
        return "✓ Root directory is clean (no stray files or undocumented tool directories)"
            .to_string();
    }

    let mut output = String::new();
    output.push_str(&format!(
        "✗ Found {} root cleanliness violations:\n",
        violations.len()
    ));
    ...
    for violation in violations {
        ...
        match violation.violation_type {
            ViolationType::StrayFile => {
                ...
                output.push_str(
                    "    → Should be moved to 1.docs/history/ or 1.docs/plans/archive/\n",
                );
            }
            ViolationType::UndocumentedToolDir => {
                output.push_str(&format!("  - Undocumented tool directory: {}\n", path_str));
                output.push_str(
                    "    → Should be documented in README or removed if not officially supported\n",
                );
            }
        }
    }
