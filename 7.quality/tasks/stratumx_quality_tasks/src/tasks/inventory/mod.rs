...
pub fn run(ctx: &QualityContext) -> Result<(), String> {
    ...
    println!("{}", cleanliness::format_violations(&cleanliness_violations));
    
    if !cleanliness_violations.is_empty() {
        return Err(format!("Root cleanliness check failed with {} violations", cleanliness_violations.len()));
    }

    // Check file size discipline
    let file_size_violations = file_size_discipline::check_file_size_discipline(&ctx.repo_root)?;
    println!("{}", file_size_discipline::format_violations(&file_size_violations));
    
    // Count unjustified violations
    let unjustified_count = file_size_violations.iter().filter(|v| !v.has_justification).count();
    if unjustified_count > 0 {
        return Err(format!("File size discipline check failed with {} unjustified violations", unjustified_count));
    }
    ...
    println!("{}", test_placement::format_violations(&test_placement_violations));
    
    if !test_placement_violations.is_empty() {
        return Err(format!("Test placement check failed with {} misplaced heavy test(s)", test_placement_violations.len()));
    }
