...
fn environment_authoring_service_updates_truth_and_bindings() {
    ...
    service.configure_sky([0.5, 0.7, 1.0], 1.0).unwrap();
    ...
    assert_eq!(environment.ambient_light, [0.3, 0.3, 0.3]);
    assert_eq!(environment.weather_condition, WeatherCondition::Rain);
