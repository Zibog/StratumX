...
use stratumx_editor_state_containers::{
    ...
    SelectionMode, ToolMode, WorldIdentity, PanelId,
};
use uuid::Uuid;
...
fn test_empty_state_serialization() {
    ...
    let deserialized = SessionState::deserialize(&serialized).expect("Deserialization should succeed");
...
fn test_default_state_equals_new_state() {
    ...
    assert_eq!(new_state.get_active_world(), default_state.get_active_world());
...
fn test_nil_entity_id_rejected() {
    let state = SessionState::new();
    let modification = StateModification::AddToSelection(Uuid::nil());
...
fn test_multiple_nil_entities_in_selection_rejected() {
    ...
    let result = state.validate_modification(&modification);
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().code, ValidationErrorCode::InvalidEntityId);
}
...
fn test_remove_entity_not_in_selection() {
    ...
    let modification = StateModification::RemoveFromSelection(entity);
    assert!(state.validate_modification(&modification).is_ok());
    assert!(state.apply_modification(modification).is_ok());
    assert!(!state.get_selection_state().has_selection());
}
...
fn test_selection_mode_transitions() {
    let mut state = SessionState::new();
    assert_eq!(state.get_selection_state().get_selection_mode(), SelectionMode::Single);
...
fn test_read_only_access_methods() {
    ...
    state.apply_modification(StateModification::SetActiveWorld(Some(world.clone()))).unwrap();
    state.apply_modification(StateModification::AddOpenPanel(PanelId("viewport".to_string()))).unwrap();
    state.apply_modification(StateModification::SetToolMode(ToolMode::Move)).unwrap();
...
fn test_selection_state_read_only_methods() {
    ...
    state.apply_modification(StateModification::AddToSelection(entity)).unwrap();
...
fn test_state_unchanged_on_validation_failure() {
    ...
    let invalid_modification = StateModification::AddToSelection(Uuid::nil());
...
fn test_validate_before_apply_pattern() {
    let mut state = SessionState::new();
    let entity = Uuid::new_v4();
    let modification = StateModification::AddToSelection(entity);
