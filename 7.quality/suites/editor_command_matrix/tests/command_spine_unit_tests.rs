...
use stratumx_editor_l7_0_editor_command_spine::{Command, CommandSpine, CommandType};
...
fn test_undo_redo_sequence() {
    ...
    spine.dispatch_command(cmd1).unwrap();
    spine.dispatch_command(cmd2).unwrap();
    spine.dispatch_command(cmd3).unwrap();
...
fn test_undo_all_then_redo_all() {
    ...
    // Execute three commands
    spine
        .dispatch_command(Command::new(CommandType::SelectEntity))
        .unwrap();
    spine
        .dispatch_command(Command::new(CommandType::TranslateEntity))
        .unwrap();
    spine
        .dispatch_command(Command::new(CommandType::RotateEntity))
        .unwrap();
...
fn test_failed_command_not_in_history() {
    ...
    let result = spine.dispatch_command(command);
...
fn test_preconditions_project_required() {
    ...
    for cmd_type in commands {
        ...
        assert!(result
            ...
            .contains(&"No project loaded".to_string()));
    }
}
...
fn test_preconditions_world_required() {
    ...
    for cmd_type in commands {
        ...
        assert!(result
            .unwrap_err()
            .failed_preconditions
            .contains(&"No active world".to_string()));
    }
}
...
fn test_preconditions_selection_required() {
    ...
    for cmd_type in commands {
        ...
        assert!(result
            .unwrap_err()
            .failed_preconditions
            .contains(&"No entities selected".to_string()));
    }
}
