...
mod tests {
    use crate::{
        MaterialProfileId, MaterialRegistryState, ProjectIdentity, ProjectState,
        WorkspaceIdentity, WorldIdentity, WorldState,
    };
    ...
    fn identity_operation_strategy() -> impl Strategy<Value = IdentityOperation> {
        prop_oneof![
            ...
            (any::<usize>(), "[a-z]{3,10}", "[a-z/]{5,20}", "[a-z0-9]{5,15}").prop_map(
                |(registry_index, name, path, snapshot_ref)| {
    ...
    fn test_material_registry_state_uses_registry_profile_ids() {
        ...
        let profile = crate::MaterialProfile::new(profile_id.clone(), "Test Profile".to_string());
        ...
        assert_eq!(*retrieved_profile.unwrap().profile_id.as_uuid(), registry_uuid);
    }
}
